...
for data in "${datasets[@]}"; do
  ...
  ./cmake-build-debug/test/test_index_time -g ${graph_path} -i ${index_path} -l ${logger_path} -s ${result_path} -m 5 -t 100

  logger_path="./DATA/${data}/${data}-index.log"
  result_path="./result/${data}/${data}-index-parallel.log"
  index_path="./DATA/${data}/${data}.index"
  ./cmake-build-debug/test/test_index_time -g ${graph_path} -i ${index_path} -l ${logger_path} -s ${result_path} -m 7 -t 100
